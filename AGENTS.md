...
- Separate Supabase request helpers into explicit service-role, anonymous, and
  authenticated-user paths.
- Use the authenticated user's access token for user-owned listing and
  showing-request operations that will later be enforced by RLS.
- Preserve service-role access only for operations that genuinely require
  administrative privileges, such as listing approval.
- Add tests proving each helper sends the intended API key and bearer token and
  fails closed when the required credentials are unavailable.
- Keep this slice narrow: do not enable RLS until request-path tests and a
  dedicated RLS integration strategy are ready.
...
1. Finish remaining UX bugs found in manual testing.
2. Separate service-role, anonymous, and authenticated-user Supabase request paths.
3. Add RLS integration tests, then implement RLS.
4. Introduce client accounts.
5. Add favorites, inquiry history, showing-request statuses, and multi-photo galleries.
...
- Recommended next engineering task: separate service-role, anonymous, and
  authenticated-user Supabase request paths before RLS implementation.
...
- Listing read tests cover successful Supabase reads, development fallback,
  ...
  configuration.
- The full Vitest suite passes with 40 tests.
- `npm run build` passes.
- Recommended next engineering task: run and verify the prepared UUID
  ownership migration before separating Supabase request paths for RLS.
